...
const initialState: StateType = {
  users: [
    // {id: 1, followed: false, name: 'Egor', status: "Hello", location: {city: 'Yaroslavl', country: 'Russia'}},
    // {id: 2, followed: true, name: 'Julia', status: "Hello", location: {city: 'Yaroslavl', country: 'Russia'}},
      ...
      return {
        ...
        users: state.users.map((el) =>
          el.id === action.payload.id
            ? { ...el, followed: !action.payload.followed }
            : el
        ),
      };
    }
...
export const unFollowTC =
  (id: number, followed: boolean) => (dispatch: Dispatch) => {
